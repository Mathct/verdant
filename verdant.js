...
define(["dojo", "dojo/_base/declare", "ebg/core/gamegui", "ebg/counter", g_gamethemeurl + "modules/js/Core/game.js"], function (dojo, declare) {
  /* CONSTANTS HERE */
  ...
  return declare("bgagame.verdant", [customgame.game], {
        ...
        if (evt.currentTarget.classList.contains("selectable") || evt.currentTarget.classList.contains("selectable_thumb_pannel")) {
          ...
          if (!evt.currentTarget.closest('[id^="market_cell_"]')) {
            ...
            if (tileMarketCount == 1 && (plantMarketCount == 1) ^ (roomMarketCount == 1)) {
              ...
              dojo.addClass("validate_mixed", "disabled");
              dojo.addClass("validate_verdancy", "disabled");
            } else if (tileMarketCount == 0 && plantMarketChildCount == 0 && roomMarketChildCount == 0 && (plantMarketNoChildCount > 0 || roomMarketNoChildCount > 0)) {
    ...
    setupBoard: function () {
      ...
      Object.values(this.gamedatas.pots).forEach((pot) => {
        if (["deck", "discard", "market"].includes(pot.location)) return;
    ...
    addCardToMarket: function (card, genre) {
      console.log("addCardToMarket", card);
      console.log("addCardToMarket", genre);
      ...
      console.log("addCardToMarket", `market_cell_${row}${col}`);
    ...
    `;
      ...
      const container = room_type === 99 ? document.getElementById(`icon_reserve_${player_id}`) : document.getElementById(`room_${room_type}`);
    ...
    animateAndRemoveVerdancy: function (plant_type) {
      //checked
      ...
      if (this.instantaneousMode) {
        ...
        return new Promise((resolve) => {
          ...
          verdancyElement.addEventListener(
            "animationend",
            () => {
              verdancyElement.remove();
            ...
            { once: true },
          ); // `once: true` pour éviter plusieurs déclenchements
        });
      }
    },
    ...
    moveBackThumbs: async function () {
      ...
      await Promise.all([processThumbs("plant", "market_cell_25"), processThumbs("room", "market_cell_45")]);
    },
    ...
    showHelpModal: function () {
      ...
      html +=
        ...
        _("Cards must be placed orthogonally adjacent to other cards. Plant Cards must be placed next to Room Cards, and vice versa, in a checkerboard pattern.") +
        "</div>";
      ...
      html +=
        "<div class='tooltip_desc'>" +
        _(
          "If a match between the Lighting condition on a Room any any of the preferred ones ont the Plant Card is made when placing a Card, then 1 Verdancy is added to the Plant Card.",
        ) +
        "</div>";
    ...
    showHelpModal2: function () {
      ...
      html +=
        "<div class='tooltip_desc'>" + _("Points indicated below ") + "<span class='inline-icon verdancy' ></span>" + _(" requirement for each completed Plant Card.") + "</div>";

      html += "<br><div class='tooltip_subtitle'>" + _("Extra Verdancy") + "</div>";
      html +=
        "<div class='tooltip_desc'>" +
        _("Each player scores half of the total number of ") +
        "<span class='inline-icon verdancy' ></span>" +
        _(" on incomplete plants.") +
        "</div>";
    ...
    notif_refillMarket: async function (args) {
      ...
      await Promise.all([this.flipAndDrawCard(args.card_pick), this.wait(100).then(() => this.drawTile(args.tile_pick))]);
    },
    ...
    notif_resetTiles: async function (args) {
      ...
      const discardPromises = args.old_tiles.map((tile, index) => this.wait(index * 100).then(() => this.animateDiscardTile(tile)));
      ...
      const drawPromises = args.new_tiles.map((tile, index) => this.wait(index * 300).then(() => this.drawTile(tile)));
    ...
    notif_resetCards: async function (args) {
      ...
      const discardPromises = Object.entries(args.old_cards).map(([index, card]) => this.wait(index * 200).then(() => this.resetCard(card)));
      await Promise.all(discardPromises);
